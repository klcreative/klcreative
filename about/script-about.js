...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 檢測是否為 Mac 設備（包括 MacBook、iMac 等）
    const isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;
    
    // 根據設備調整 Lenis 配置
    const lenis = new Lenis({
        duration: isMac ? 0.8 : 1.2,  // Mac 設備使用更短的動畫時間
        easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smooth: true,
        smoothTouch: false,  // 在觸控設備上禁用平滑滾動
        touchMultiplier: 2,  // 增加觸控靈敏度
        wheelMultiplier: isMac ? 1.5 : 1,  // Mac 設備增加滾輪靈敏度
        infinite: false
    });
