...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cards.forEach(card => {
        ...
        const leftText = card.querySelector('.card-left-text');
        if (leftText) {
            cardFront.appendChild(leftText.cloneNode(true));
        }
        
        // 創建背面
        const cardBack = document.createElement('div');
        cardBack.className = 'card-back';
        const rightText = card.querySelector('.card-right-text');
        if (rightText) {
            cardBack.appendChild(rightText.cloneNode(true));
        }
