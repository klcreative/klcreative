...
document.addEventListener('DOMContentLoaded', () => {
    ...
    parallaxSections.forEach((section, sectionIndex) => {
        ...
        if (img3 && img4) {
            ...
            // 圖片視差效果 - 圖片3（人物）移動慢（-20px），圖片4（場景）移動快（-60px）
            // 但圖片4的移動範圍限制，確保人物底部不露出
            gsap.to(img3, {
                scrollTrigger: {
                    trigger: section,
                    start: 'top bottom',
            ...
            gsap.to(img4, {
                scrollTrigger: {
                    ...
                    scrub: 1.5,
                    onUpdate: (self) => {
                        // 限制圖片4的移動，確保不超過圖片3的底部
                        const maxMove = -40; // 最大向上移動40px，確保不露出人物底部
                        const targetY = -60 * self.progress;
                        const clampedY = Math.max(targetY, maxMove);
                        gsap.set(img4, { y: clampedY });
                    }
                },
                ease: 'none'
            });
