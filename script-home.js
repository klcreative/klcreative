/* Version: 2026-0129-UPDATE - Smooth Continuous Parallax */
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========== 改良版視差滾動 - 持續平滑更新 ==========
    const PARALLAX_SPEED = 0.5; // 視差滾動速度
    
    function updateParallaxSmooth() {
    ...
    // === Enable scroll monitoring with smooth updates ===
    let ticking = false;
    ...
    const contentObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            const container = entry.target.querySelector('.content-container');
            if (!container) return;
    ...
    if (logoSection) {
        ...
        setTimeout(() => {
            ...
            // Restore normal state, let parallax take over
            if (parallaxBg) {
