        ...
        (entries) => {
            entries.forEach(entry => {
                ...
                if (window.innerWidth > 768 || window.matchMedia("(orientation: landscape)").matches) {
                    ...
                    // 修正 baseScale: Desktop: 110 (原 105), Landscape: 150 (原 130)
                    const baseScale = isLandscapeMobile ? 150 : 110; 
                    // 修正 variableScale: 增加到 25 (原 15)，使視差更明顯
                    const variableScale = 25; 
    ...
    function updateDesktopParallax() {
        ...
        if (!parallaxTicking) {
            requestAnimationFrame(() => {
                const isLandscapeMobile = window.matchMedia("(orientation: landscape)").matches && window.innerHeight < 500;
                // Base scale: Desktop: 110 (原 105), Landscape: 150 (原 130)
                const baseScale = isLandscapeMobile ? 150 : 110;
                // Variable scale: 增加到 25 (原 15)，使視差更明顯
                const variableScale = 25;
