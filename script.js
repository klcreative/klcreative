...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mainObserver = new IntersectionObserver(
        ...
        },
        {
            root: null,
            threshold: 0.1, 
            rootMargin: '0px 0px -30% 0px' // 調整為-30% 以提早觸發效果
        }
    );
    ...
    if (logoSection) {
        // 延遲一段時間執行，確保頁面載入完成
        setTimeout(() => {
            logoSection.classList.remove('unblurred');
        }, 100);
    }
    
    const logoImage = document.querySelector('.logo-animation-container img');
    if (logoImage) {
        logoImage.classList.add('scaled');
    }
});