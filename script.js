...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 建立一個專門用於處理導覽列的 Intersection Observer
    ...
    if (logoSection) {
        const logoAnimationContainer = logoSection.querySelector('.logo-animation-container');
        const overlay = logoSection.querySelector('.logo-overlay'); // 修正: 選取毛玻璃遮罩元素
        
        // 延遲一段時間後執行動畫
        setTimeout(() => {
            logoSection.classList.add('fade-out-overlay'); // 移除毛玻璃
            logoSection.classList.add('unblurred'); // 觸發背景放大
        }, 500); // 延遲0.5秒後啟動動畫
    }
});