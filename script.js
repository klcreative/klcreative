...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const slideOutObserver = new IntersectionObserver(
        ...
        {
            root: null,
            threshold: 0.01, // 進一步降低觸發閾值
            rootMargin: '100px 0px' // 大幅擴大觸發範圍，讓滑出效果更早更廣觸發
        }
    );

    // 背景放大效果 Observer - 同時處理footer
    const backgroundObserver = new IntersectionObserver(
        (entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.remove('unblurred');
                    // 如果是contact區塊，也控制footer的背景
                    if (entry.target.id === 'contact') {
                        const footer = document.getElementById('main-footer');
                        if (footer) footer.classList.remove('unblurred');
                    }
                } else {
                    ...
                    if (entry.target.id === 'contact') {
                        const footer = document.getElementById('main-footer');
                        if (footer) footer.classList.add('unblurred');
                    }
                }
            });
        },
