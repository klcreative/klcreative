...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // === 視差效果 - translateY 移動 (所有設備) ===
    let parallaxTicking = false;
    function updateParallaxTranslate() {
        if (!parallaxTicking) {
            requestAnimationFrame(() => {
                const scrollY = window.pageYOffset;
                
                // 處理所有區塊的視差背景 translateY
                ...
                // 處理 contact-footer-wrapper 的視差背景 translateY
    ...
    let desktopScaleTicking = false;
    function updateDesktopBackgroundScale() {
        // 排除手機直式
        if (isPortraitMobile()) return;
        
        if (!desktopScaleTicking) {
            requestAnimationFrame(() => {
                const baseScale = isLandscapeMobile() ? 140 : 105;
                const variableScale = isLandscapeMobile() ? 15 : 15;
                ...
                desktopScaleTicking = false;
            });
            desktopScaleTicking = true;
        }
    }
    ...
    // === 啟用滾動監聽 ===
    // 1. 視差 translateY - 所有設備
    window.addEventListener('scroll', updateParallaxTranslate, { passive: true });
    
    // 2. 背景縮放 - 桌面版 & 手機橫式
    window.addEventListener('scroll', updateDesktopBackgroundScale, { passive: true });
