...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 新的背景縮放觀察器 - 更精細的三等份控制 (雙欄樣式修改:雙向觸發)
                    ...
                    if (section.classList.contains('loaded')) {
                        ...
                        if (window.innerWidth <= 768) {
                            ...
                            if (sectionTop > windowHeight * 0.3) {
                                ...
                                const centerProgress = Math.abs(sectionTop + sectionHeight/2 - windowHeight/2) / (windowHeight/2);
                                mobileScale = 1.0 + (centerProgress * 0.8);
                            }
    ...
    const contentObserver = new IntersectionObserver(
        ...
        {
            root: null,
            threshold: 0.35, // 雙欄樣式:提早觸發滑入
            rootMargin: '15% 0px -50% 0px' // 雙欄樣式:往下沉動提早滑出
        }
    );
    ...
    let selectedObserver = contentObserver; // 預設雙欄樣式
    if (isMobile) {
    ...
    if (mobileLandscapeNavOverlay) {
        mobileLandscapeNavOverlay.addEventListener('click', function(e) {
            if (e.target === mobileLandscapeNavOverlay) {
                ...
                mobileLandscapeNavToggle.blur(); // 移除焦點避免方框
            }
        });
    }
            ...
            requestAnimationFrame(() => {
                contentSections.forEach(section => {
                    ...
                    if (rect.bottom > 0 && rect.top < windowHeight) {
                        ...
                        // 手機橫式:背景圖片縮放效果(比照雙欄樣式)
                        else if (window.innerWidth > 768 && window.innerWidth <= 932 && window.matchMedia("(orientation: landscape)").matches) {
                            // 使用雙欄樣式的background-size縮放方式,不使用transform
                        }
                    }
                });
