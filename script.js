...
document.addEventListener('DOMContentLoaded', () => {
        ...
        (entries) => {
            entries.forEach(entry => {
                ...
                if (window.innerWidth <= 768 || (window.innerWidth <= 1024 && window.orientation === 0)) {
                    let currentScale = scale; // 新增變數來控制手機版的縮放值
                    
                    if (section.id === 'logo-section' && !section.classList.contains('loaded')) {
                        // MODIFIED: 在初始載入動畫完成前，將縮放值固定為 1.8，以執行初始縮放動畫 (1.8 -> 1.0)
                        currentScale = 1.8;
                    }
                    
                    // MODIFIED: 讓所有區塊使用 var(--bg-scale) 實現連續縮放 (包括 logo section)
                    section.style.setProperty('--bg-scale', currentScale);
                }
            });
        },
        {
    ...
    if (logoSection) {
        ...
        setTimeout(() => {
            ...
            logoSection.classList.add('loaded'); // 啟動從1.8到1.0的縮放（由 var(--bg-scale) 控制）
        }, 500);
    }

    // 滾動時更新背景縮放（額外的滾動監聽 - 主要為電腦版或非 CSS 變數控制時使用）
    let ticking = false;
