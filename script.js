    ...
    const backgroundScaleObserver = new IntersectionObserver(
        (entries) => {
            entries.forEach(entry => {
                ...
                // 當區域接近視窗中心時縮小，遠離時放大（視差效果）
                let scale = 105 + (progress * 15); // 從105%到120%
    ...
    function updateMobileParallax() {
        ...
        if (logoSection && logoSection.classList.contains('loaded')) {
            ...
            if (rect.bottom > 0 && rect.top < windowHeight) {
                ...
                // 使用 ::before 偽元素的 transform
                const beforeElement = window.getComputedStyle(logoSection, '::before');
                logoSection.style.setProperty('--logo-bg-scale', mobileScale);
            }
        }
