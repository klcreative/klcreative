...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mobileLandscapeNavMenu = document.getElementById('mobile-landscape-nav-menu');
    ...
    if (mobileNavToggle && mobileNavOverlay) {
        mobileNavToggle.addEventListener('click', function() {
            const isOpen = mobileNavOverlay.style.display === 'flex';
            mobileNavOverlay.style.display = isOpen ? 'none' : 'flex';
    ...
    if (mobileLandscapeNavToggle && mobileLandscapeNavOverlay) {
        mobileLandscapeNavToggle.addEventListener('click', function() {
            const isOpen = mobileLandscapeNavOverlay.style.display === 'flex';
            mobileLandscapeNavOverlay.style.display = isOpen ? 'none' : 'flex';
            mobileLandscapeNavToggle.setAttribute('aria-expanded', !isOpen);
            
            if (!isOpen) {
                const firstLink = mobileLandscapeNavMenu.querySelector('a');
                if (firstLink) {
                    setTimeout(() => firstLink.focus(), 100);
                }
            } else {
                mobileLandscapeNavToggle.blur();
            }
        });
    }
    ...
    if (mobileLandscapeNavOverlay) {
        mobileLandscapeNavOverlay.addEventListener('click', function(e) {
            if (e.target === mobileLandscapeNavOverlay) {
                mobileLandscapeNavOverlay.style.display = 'none';
                mobileLandscapeNavToggle.setAttribute('aria-expanded', 'false');
                mobileLandscapeNavToggle.blur();
            }
        });
    }
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' || e.key === 'Esc') {
            if (mobileNavOverlay && mobileNavOverlay.style.display === 'flex') {
                ...
                setTimeout(() => mobileNavToggle.blur(), 100);
            }
            if (mobileLandscapeNavOverlay && mobileLandscapeNavOverlay.style.display === 'flex') {
                mobileLandscapeNavOverlay.style.display = 'none';
                mobileLandscapeNavToggle.setAttribute('aria-expanded', 'false');
                mobileLandscapeNavToggle.focus();
                setTimeout(() => mobileLandscapeNavToggle.blur(), 100);
            }
        }
    });
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Tab') {
            if (mobileNavOverlay && mobileNavOverlay.style.display === 'flex') {
                const focusableElements = mobileNavMenu.querySelectorAll('a');
            ...
            if (mobileLandscapeNavOverlay && mobileLandscapeNavOverlay.style.display === 'flex') {
                const focusableElements = mobileLandscapeNavMenu.querySelectorAll('a');
                const firstElement = focusableElements[0];
                const lastElement = focusableElements[focusableElements.length - 1];
    ...
    function updateMobileParallax() {
        const isMobileView = window.innerWidth <= 932;
        const isLandscapeView = window.innerWidth > 768 && window.innerWidth <= 932 && window.matchMedia("(orientation: landscape)").matches;
    ...
    if (logoSection) {
        ...
        // Logo 背景顏色完全透明修正 - 移除多餘的 overlay
        const logoBeforeElement = window.getComputedStyle(logoSection, '::before');
        logoSection.style.setProperty('--logo-blur-amount', '0px');
    }
    ...
    window.addEventListener('resize', () => {
        const newIsLandscape = window.innerWidth > 768 && window.innerWidth <= 932 && window.matchMedia("(orientation: landscape)").matches;
        const newIsMobile = window.innerWidth <= 932;
    ...
    // 確保頁面載入時滾動到頂部 (針對手機橫式)
    if (window.innerWidth > 768 && window.innerWidth <= 932 && window.matchMedia("(orientation: landscape)").matches) {
        window.scrollTo(0, 0);
    }
});